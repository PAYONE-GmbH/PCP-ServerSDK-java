...
public class Customer implements Serializable {
  ...
  public String toUrlQueryString(String prefix) {
    ...
    if (getFiscalNumber() != null) {
      try {
        joiner.add(String.format("%sfiscalNumber%s=%s", prefix, suffix,
            URLEncoder.encode(String.valueOf(getFiscalNumber()), "UTF-8").replaceAll("\\+", "%20")));
      } catch (UnsupportedEncodingException e) {
        // Should never happen, UTF-8 is always supported
        throw new RuntimeException(e);
      }
    ...
    if (getBusinessRelation() != null) {
      try {
        joiner.add(String.format("%sbusinessRelation%s=%s", prefix, suffix,
            URLEncoder.encode(String.valueOf(getBusinessRelation()), "UTF-8").replaceAll("\\+", "%20")));
      } catch (UnsupportedEncodingException e) {
        // Should never happen, UTF-8 is always supported
        throw new RuntimeException(e);
      }
