...
public class MobilePaymentMethodSpecificInput implements Serializable {
  ...
  public String toUrlQueryString(String prefix) {
    ...
    if (getEncryptedPaymentData() != null) {
      try {
        joiner.add(String.format("%sencryptedPaymentData%s=%s", prefix, suffix,
            URLEncoder.encode(String.valueOf(getEncryptedPaymentData()), "UTF-8").replaceAll("\\+", "%20")));
      } catch (UnsupportedEncodingException e) {
        // Should never happen, UTF-8 is always supported
        throw new RuntimeException(e);
      }
    ...
    if (getPublicKeyHash() != null) {
      try {
        joiner.add(String.format("%spublicKeyHash%s=%s", prefix, suffix,
            URLEncoder.encode(String.valueOf(getPublicKeyHash()), "UTF-8").replaceAll("\\+", "%20")));
      } catch (UnsupportedEncodingException e) {
        // Should never happen, UTF-8 is always supported
        throw new RuntimeException(e);
      }
    ...
    if (getEphemeralKey() != null) {
      try {
        joiner.add(String.format("%sephemeralKey%s=%s", prefix, suffix,
            URLEncoder.encode(String.valueOf(getEphemeralKey()), "UTF-8").replaceAll("\\+", "%20")));
      } catch (UnsupportedEncodingException e) {
        // Should never happen, UTF-8 is always supported
        throw new RuntimeException(e);
      }
    ...
    if (getPaymentProduct302SpecificInput() != null) {
      joiner.add(
          getPaymentProduct302SpecificInput().toUrlQueryString(prefix + "paymentProduct302SpecificInput" + suffix));
    }
