...
public class ProcessingMandateInformation implements Serializable {
  ...
  public String toUrlQueryString(String prefix) {
    ...
    if (getRecurrenceType() != null) {
      try {
        joiner.add(String.format("%srecurrenceType%s=%s", prefix, suffix,
            URLEncoder.encode(String.valueOf(getRecurrenceType()), "UTF-8").replaceAll("\\+", "%20")));
    ...
    if (getUniqueMandateReference() != null) {
      try {
        joiner.add(String.format("%suniqueMandateReference%s=%s", prefix, suffix,
            URLEncoder.encode(String.valueOf(getUniqueMandateReference()), "UTF-8").replaceAll("\\+", "%20")));
      } catch (UnsupportedEncodingException e) {
        // Should never happen, UTF-8 is always supported
        throw new RuntimeException(e);
      }
    ...
    if (getDateOfSignature() != null) {
      try {
        joiner.add(String.format("%sdateOfSignature%s=%s", prefix, suffix,
            URLEncoder.encode(String.valueOf(getDateOfSignature()), "UTF-8").replaceAll("\\+", "%20")));
      } catch (UnsupportedEncodingException e) {
        // Should never happen, UTF-8 is always supported
        throw new RuntimeException(e);
      }
    ...
    if (getCreditorId() != null) {
      try {
        joiner.add(String.format("%screditorId%s=%s", prefix, suffix,
            URLEncoder.encode(String.valueOf(getCreditorId()), "UTF-8").replaceAll("\\+", "%20")));
      } catch (UnsupportedEncodingException e) {
        // Should never happen, UTF-8 is always supported
        throw new RuntimeException(e);
      }
