...
public class MobilePaymentMethodSpecificOutput implements Serializable {
  ...
  public String toUrlQueryString(String prefix) {
    ...
    if (getAuthorisationCode() != null) {
      try {
        joiner.add(String.format("%sauthorisationCode%s=%s", prefix, suffix,
            URLEncoder.encode(String.valueOf(getAuthorisationCode()), "UTF-8").replaceAll("\\+", "%20")));
      } catch (UnsupportedEncodingException e) {
        // Should never happen, UTF-8 is always supported
        throw new RuntimeException(e);
      }
    ...
    if (getNetwork() != null) {
      try {
        joiner.add(String.format("%snetwork%s=%s", prefix, suffix,
            URLEncoder.encode(String.valueOf(getNetwork()), "UTF-8").replaceAll("\\+", "%20")));
      } catch (UnsupportedEncodingException e) {
        // Should never happen, UTF-8 is always supported
        throw new RuntimeException(e);
      }
