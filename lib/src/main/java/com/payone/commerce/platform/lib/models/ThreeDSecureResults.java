...
public class ThreeDSecureResults implements Serializable {
  ...
  public String toUrlQueryString(String prefix) {
    ...
    if (getSchemeEci() != null) {
      try {
        joiner.add(String.format("%sschemeEci%s=%s", prefix, suffix,
            URLEncoder.encode(String.valueOf(getSchemeEci()), "UTF-8").replaceAll("\\+", "%20")));
      } catch (UnsupportedEncodingException e) {
        // Should never happen, UTF-8 is always supported
        throw new RuntimeException(e);
      }
    ...
    if (getAppliedExemption() != null) {
      try {
        joiner.add(String.format("%sappliedExemption%s=%s", prefix, suffix,
            URLEncoder.encode(String.valueOf(getAppliedExemption()), "UTF-8").replaceAll("\\+", "%20")));
      } catch (UnsupportedEncodingException e) {
        // Should never happen, UTF-8 is always supported
        throw new RuntimeException(e);
      }
