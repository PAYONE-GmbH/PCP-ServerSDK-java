...
public class CheckoutResponse implements Serializable {
  ...
  **/
  ...
  @Override
  public int hashCode() {
    return Objects.hash(commerceCaseId, checkoutId, merchantCustomerId, amountOfMoney, references, shipping, shoppingCart, paymentExecutions, checkoutStatus, statusOutput, paymentInformation, creationDateTime, allowedPaymentActions);
  }
    ...
    if (getPaymentExecutions() != null) {
      for (int i = 0; i < getPaymentExecutions().size(); i++) {
        if (getPaymentExecutions().get(i) != null) {
          joiner.add(getPaymentExecutions().get(i).toUrlQueryString(String.format("%spaymentExecutions%s%s", prefix, suffix,
    ...
    if (getPaymentInformation() != null) {
      for (int i = 0; i < getPaymentInformation().size(); i++) {
        if (getPaymentInformation().get(i) != null) {
          joiner.add(getPaymentInformation().get(i).toUrlQueryString(String.format("%spaymentInformation%s%s", prefix, suffix,
              "".equals(suffix) ? "" : String.format("%s%d%s", containerPrefix, i, containerSuffix))));
        }
      }
    }
